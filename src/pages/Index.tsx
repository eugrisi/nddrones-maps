...
const Index = () => {
  ...
  return (
    ...
    >
      ...
      {/* Controles Flutuantes - Reorganizados em linha horizontal */}
      <div className="fixed top-6 right-6 z-[9999] print:hidden">
        <div className="flex items-center space-x-3">
        {/* Toggle Modo Noturno */}
        <button
          onClick={() => setDarkMode(!darkMode)}
          className={`p-2 rounded-full shadow-lg transition-all hover:scale-110 ${
            darkMode ? 'bg-yellow-500 text-white' : 'bg-gray-800 text-yellow-400'
          }`}
        ...
        >
          ...
          <div className="relative group">
            ...
            <div className={`absolute right-0 top-full mt-2 w-80 rounded-lg shadow-xl border opacity-0 invisible group-hover:opacity-100 group-hover:visible transition-all duration-200 z-50 ${
      ...
      {/* Sidebar ND Drones - Oculta na impressão */}
      <aside className={`${sidebarOpen ? 'w-72' : 'w-0'} ${darkMode ? 'bg-gray-800 text-gray-100' : 'bg-nd-green-dark text-nd-beige'} flex flex-col shadow-2xl transition-all duration-300 overflow-hidden print:hidden`}>
        {/* Logo */}
        <div className={`p-4 border-b ${darkMode ? 'border-gray-700' : 'border-nd-green-light/30'} flex items-center justify-center`}>
          <img 
            src={custom.logo} 
            alt="ND Drones" 
            className="object-contain mx-auto transition-transform hover:scale-110"
            style={{ width: '60px', height: '60px', minWidth: '40px', minHeight: '40px' }}
          />
        </div>
        ...
        <div className={`p-4 space-y-4 border-b ${darkMode ? 'border-gray-700' : 'border-nd-green-light/30'}`}>
          <div className="space-y-3">
              ...
              <select
                className={`w-full border rounded-md px-3 py-2 text-sm focus:outline-none focus:ring-1 focus:ring-nd-orange focus:border-transparent transition-all ${
                  darkMode 
                    ...
                    : 'bg-white/10 border-nd-green-light/30 text-nd-beige'
                }`}
            ...
            <div className="grid grid-cols-2 gap-2">
              <button
                className="bg-nd-orange hover:bg-nd-orange/90 text-white font-bold py-2 px-3 rounded-md text-sm transition-all transform hover:scale-105 focus:outline-none focus:ring-1 focus:ring-nd-orange"
              ...
              <button
                className={`font-bold py-2 px-3 rounded-md text-sm transition-all transform hover:scale-105 focus:outline-none focus:ring-1 ${
                  darkMode 
                    ? 'bg-gray-600 hover:bg-gray-500 text-gray-100 focus:ring-gray-600' 
                    : 'bg-nd-green-dark hover:bg-nd-green-light text-nd-beige focus:ring-nd-green-dark'
                }`}
      ...
      <main className="map-container flex-1 relative">
        ...
        <div className="h-full m-4 rounded-2xl overflow-hidden shadow-2xl border border-nd-green-light/20 print:m-0 print:rounded-none print:shadow-none print:border-none print:w-screen print:h-screen">
          ...
          {/* Controles de Tipo de Mapa - Canto Inferior Esquerdo */}
          <div className="absolute bottom-20 left-6 z-[1000] print:hidden">
            <div className="bg-black/90 backdrop-blur-sm rounded-lg shadow-2xl border border-gray-700 overflow-hidden">
              <div className="flex flex-col">
                <button 
                  onClick={() => setMapType('traditional')}
                  className={`px-4 py-3 text-sm font-medium transition-all border-b border-gray-700 ${
                    mapType === 'traditional'
                      ? 'bg-white text-black' 
                ...
                >
                  <svg className="w-4 h-4 inline mr-2" fill="currentColor" viewBox="0 0 24 24">
                    <path d="M12 2L13.09 8.26L22 9L13.09 9.74L12 16L10.91 9.74L2 9L10.91 8.26L12 2M12 8C10.9 8 10 8.9 10 10S10.9 12 12 12 14 11.1 14 10 13.1 8 12 8Z"/>
                  </svg>
                  Satélite
                </button>
              </div>
